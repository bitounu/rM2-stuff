...
static inline int
sixel_bitmap(struct terminal_t* term,
  ...
  for (i = 0; i < BITS_PER_SIXEL; i++) {
    ...
    if (bitmap & (0x01 << i)) {
      memcpy(sc->pixmap + offset,
             &sc->color_table[sc->color_index],
             BYTES_PER_PIXEL);
      /*
...
static inline int
sixel_repeat(struct terminal_t* term, struct sixel_canvas_t* sc, char* buf) {
  int i, count;
  size_t length;
  char *cp, tmp[BUFSIZE];
  uint8_t bitmap;
...
static inline int
sixel_attr(struct sixel_canvas_t* sc, char* buf) {
  char *cp, tmp[BUFSIZE];
  size_t length;
  struct parm_t parm;
...
hue2rgb(int n1, int n2, int hue) {
  ...
  if (hue < (HUEMAX / 2))
    return n2;
  if (hue < ((HUEMAX * 2) / 3))
    return (n1 + (((n2 - n1) * (((HUEMAX * 2) / 3) - hue) + (HUEMAX / 12)) /
                  (HUEMAX / 6)));
  else
    return n1;
}
...
hls2rgb(int hue, int lum, int sat) {
  ...
  if (sat == 0) {
    ...
    r = (hue2rgb(magic1, magic2, hue + (HUEMAX / 3)) * RGBMAX + (LSMAX / 2)) /
        LSMAX;
    g = (hue2rgb(magic1, magic2, hue) * RGBMAX + (LSMAX / 2)) / LSMAX;
    b = (hue2rgb(magic1, magic2, hue - (HUEMAX / 3)) * RGBMAX + (LSMAX / 2)) /
        LSMAX;
  }
  return (r << 16) + (g << 8) + b;
}
...
static inline int
sixel_color(struct sixel_canvas_t* sc, char* buf) {
  char *cp, tmp[BUFSIZE];
  int index, type;
  size_t length;
  ...
  *(tmp + length - 1) = '\0';
  ...
  logging(
    DEBUG, "type:%d v1:%u v2:%u v3:%u color:0x%.8X\n", type, v1, v2, v3, color);
...
static inline int
sixel_cr(struct sixel_canvas_t* sc) {
...
static inline int
sixel_nl(struct sixel_canvas_t* sc) {
...
void
sixel_parse_data(struct terminal_t* term,
                 struct sixel_canvas_t* sc,
                 char* start_buf) {
  /*
  DECDLD sixel data
          ...
          '"': attr
                  " Pan; Pad; Ph; Pv
                          ...
                          Ph, Pv  : defines vertical/horizontal size of the
  image
          '!': repeat
                  ! Pn ch
                          Pn : repeat count ([0-9]+)
  ...
  */
  ...
  logging(
    DEBUG, "sixel_parse_data()\nwidth:%d height:%d\n", sc->width, sc->height);
}

void
reset_sixel(struct sixel_canvas_t* sc,
  ...
  */
  sc->color_table[0] = 0x000000;
  sc->color_table[8] = 0x424242;
  sc->color_table[1] = 0x3333CC;
  sc->color_table[9] = 0x545499;
  sc->color_table[2] = 0xCC2121;
  sc->color_table[10] = 0x994242;
  sc->color_table[3] = 0x33CC33;
  sc->color_table[11] = 0x549954;
  sc->color_table[4] = 0xCC33CC;
  sc->color_table[12] = 0x995499;
  sc->color_table[5] = 0x33CCCC;
  sc->color_table[13] = 0x549999;
  sc->color_table[6] = 0xCCCC33;
  sc->color_table[14] = 0x999954;
  sc->color_table[7] = 0x878787;
  sc->color_table[15] = 0xCCCCCC;
...
void
sixel_copy2cell(struct terminal_t* term, struct sixel_canvas_t* sc) {
    ...
    for (x = 0; x < cols; x++) {
      ...
      for (h = 0; h < CELL_HEIGHT; h++) {
        ...
        if (src_offset >= BYTES_PER_PIXEL * term->width * term->height)
          break;
        memcpy(cellp->pixmap + dst_offset,
               sc->pixmap + src_offset,
               CELL_WIDTH * BYTES_PER_PIXEL);
      }
    }
...
void
sixel_parse_header(struct terminal_t* term, char* start_buf) {
  /*
  sixel format
...
static inline void
decdld_bitmap(struct glyph_t* glyph,
              uint8_t bitmap,
              uint8_t row,
              uint8_t column) {
  /*
...
static inline void
init_glyph(struct glyph_t* glyph) {
  int i;
...
void
decdld_parse_data(char* start_buf, int start_char, struct glyph_t* chars) {
  /*
  DECDLD sixel data
          ';': glyph separator
          '/': line feed
...
void
decdld_parse_header(struct terminal_t* term, char* start_buf) {
  /*
  DECDLD format
          DCS Pfn; Pcn; Pe; Pcmw; Pss; Pt; Pcmh; Pcss; f Dscs Sxbp1 ; Sxbp2 ; ..
  .; Sxbpn ST parameters DCS : ESC (0x1B) 'P' (0x50) (DCS(8bit C1 code) is not
  supported) Pfn : fontset (ignored) Pcn : start char (0 means SPACE 0x20) Pe  :
  erase mode 0: clear selectet charset 1: clear only redefined glyph 2: clear
  all drcs charset Pcmw: max cellwidth (force CELL_WEDTH defined in glyph.h) Pss
  : screen size (ignored) Pt  : defines the glyph as text or full cell or sixel
  (force full cell mode) (TODO: implement sixel/text mode) Pcmh: max cellheight
  (force CELL_HEIGHT defined in glyph.h) Pcss: character set size (force: 96) 0:
  94 glyphs charset 1: 96 glyphs charset f   : '{' (0x7B) Dscs: define character
  set Intermediate char: SPACE (0x20) to '/' (0x2F) final char       : '0'
  (0x30) to '~' (0x7E) but allow chars between '@' (0x40) and '~' (0x7E) for
  DRCSMMv1 (ref: https://github.com/saitoha/drcsterm/blob/master/README.rst)
  ...
  *cp = '\0';
  ...
  if (0x40 <= *cp &&
      *cp <=
        0x7E) /* final char of Dscs must be between 0x40 to 0x7E (DRCSMMv1) */
    charset = *cp - 0x40;
  else
    charset = 0;

  logging(DEBUG,
          "charset(ku):0x%.2X start_char:%d erase_mode:%d\n",
          charset,
          start_char,
          erase_mode);
  ...
  if (erase_mode == 2) /* reset all drcs charset */
    ...
    memset(term->drcs + GLYPHS_PER_CHARSET * charset,
           0,
           sizeof(struct glyph_t) * DRCS_CHARS);
  ...
  decdld_parse_data(cp + 1,
                    start_char,
                    term->drcs +
                      GLYPHS_PER_CHARSET * charset); /* skip final char */
}
