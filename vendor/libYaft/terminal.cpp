...
static int
set_cell(struct terminal_t* term,
         int y,
         int x,
         const struct glyph_t* glyphp,
         const struct glyph_t* boldp) {
  struct cell_t cell, *cellp;
  uint8_t color_tmp;

  cell.glyph.regularp = glyphp;
  cell.glyph.boldp = boldp;

  cell.color_pair.fg =
    /*(term->attribute & attr_mask[ATTR_BOLD] && term->color_pair.fg <= 7)
      ? term->color_pair.fg + BRIGHT_INC
      :*/
    term->color_pair.fg;
  cell.color_pair.bg =
    (term->attribute & attr_mask[ATTR_BLINK] && term->color_pair.bg <= 7)
      ? term->color_pair.bg + BRIGHT_INC
      : term->color_pair.bg;
  ...
  cell.width = (enum glyph_width)glyphp->width;
  ...
  *cellp = cell;
  ...
  if (cell.width == HALF /* isolated NEXT_TO_WIDE cell */
      && x + 1 < term->cols && term->cells[y][x + 1].width == NEXT_TO_WIDE) {
...
static inline void
swap_lines(struct terminal_t* term, int i, int j) {
  struct cell_t* tmp;
...
static const struct glyph_t*
drcs_glyph(struct terminal_t* term, uint32_t code) {
  /* DRCSMMv1
...
addch(struct terminal_t* term, uint32_t code) {
  int width;
  const struct glyph_t* glyphp;
  const struct glyph_t* boldp = nullptr;
  ...
  if (width <= 0) /* zero width: not support combining characters */
    return;
           ...
           code <= 0x10FFFD) /* unicode private area: plane 16 (DRCSMMv1) */
    ...
    glyphp = (width == 1) ? term->glyph[SUBSTITUTE_HALF]
                          : term->glyph[SUBSTITUTE_WIDE];
  else {
    glyphp = term->glyph[code];
    boldp = term->bold_glyph[code];
  }
  ...
  move_cursor(
    term, 0, set_cell(term, term->cursor.y, term->cursor.x, glyphp, boldp));
}
...
push_esc(struct terminal_t* term, uint8_t ch) {
  ...
  if ((term->esc.bp - term->esc.buf) >= term->esc.size) { /* buffer limit */
    logging(DEBUG,
            "escape sequence length >= %d, term.esc.buf reallocated\n",
            term->esc.size);
  ...
  *term->esc.bp++ = ch;
  if (term->esc.state == STATE_ESC) {
    ...
    */
    if (ch == BEL || (ch == BACKSLASH && (term->esc.bp - term->esc.buf) >= 2 &&
                      *(term->esc.bp - 2) == ESC))
      return true;
...
term_init(struct terminal_t* term, int width, int height) {
  ...
  for (uint32_t code = 0; code < UCS2_CHARS; code++) {
    ...
    term->bold_glyph[code] = NULL;
  }

  for (uint32_t gi = 0; gi < sizeof(glyphs) / sizeof(struct glyph_t); gi++)
    term->glyph[glyphs[gi].code] = &glyphs[gi];

  for (uint32_t gi = 0; gi < sizeof(bold_glyphs) / sizeof(struct glyph_t); gi++)
    term->bold_glyph[bold_glyphs[gi].code] = &bold_glyphs[gi];

  if (!term->glyph[DEFAULT_CHAR] || !term->glyph[SUBSTITUTE_HALF] ||
      !term->glyph[SUBSTITUTE_WIDE]) {
    logging(ERROR,
            "couldn't find essential glyph:\
			DEFAULT_CHAR(U+%.4X):%p SUBSTITUTE_HALF(U+%.4X):%p SUBSTITUTE_WIDE(U+%.4X):%p\n",
            DEFAULT_CHAR,
            term->glyph[DEFAULT_CHAR],
            SUBSTITUTE_HALF,
